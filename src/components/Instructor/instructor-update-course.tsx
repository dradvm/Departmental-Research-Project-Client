...
export default function CourseForm(props: CourseFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (sections.length === 0) {
            setErrors((prev) => ({
                ...prev,
                sections: [{ title: 'At least one section is required.', lectures: [] }],
            }))
            return
        }
        ...
        const sectionErrors = sections.map((section) => {
            const sectionTitleError = section.title.trim() ? '' : 'Section title is required.'
            const lectureErrors = section.lectures.map((lecture) => {
                if (!lecture.title.trim()) return 'Lecture title is required.'
                if (lecture.contents.length === 0) return 'At least one video is required.'
                return ''
            })
        ...
        const hasErrors =
            ...
            newErrors.sections.some((sec) => sec.title || sec.lectures.some((l) => l))
