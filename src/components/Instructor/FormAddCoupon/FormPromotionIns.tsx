...
import { CouponCourseBody } from "types/couponcourse";
...
export default function PromotionForm({
    ...
    //         data.isGlobal = mode === "global";

    //         const createdCoupon = await couponService.createCoupon(data);

    //         // ✅ Nếu là mã giảm cho 1 khóa học
    //         if (mode === "course" && courseId) {
    //             await couponCourseService.createACouponCourse({
    //                 courseId,
    //                 couponId: createdCoupon.data.couponId, // Giả sử backend trả về coupon với `id`
    //             });
    //         }
    ...
    const onSubmit: SubmitHandler<Omit<any, "courseId">> = async (formData) => {
        try {
            const isExistingCode: boolean = (
                await couponService.checkIsExistingCode(formData.code)
            ).data;
            if (isExistingCode) {
                handleInformIsExistingCode();
                return;
            ...
            const body: CouponCourseBody = {
                code: formData.code.trim(),
                minRequire: formData.minRequire || 0,
                maxValueDiscount: formData.maxValueDiscount || 0,
                quantity: formData.quantity || 0,
                type: formData.type as "discount" | "voucher",
            ...
            console.log("Submitting coupon data:", body);

            const isApply = await couponCourseService.checkIsApplyingCoupon(courseId!);
            console.log("Is applying coupon:", isApply.data);
            if (isApply.data) {
                handleIsApplyingCode();
                return;
            }
            await couponCourseService.createACouponCourse(body); // ✅ body đúng kiểu
            handleSuccessfulCreation();
        } catch (e) {
            handleFailedCreation();
            console.error("Lỗi khi tạo coupon:", e);
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (confirm(`Chưa lưu thay đổi. Vẫn muốn quay lại???`)) {
                                if (onCancel) onCancel(); // ✅ gọi hàm đóng modal
                            }
                        }}
                    >
                        Hủy
